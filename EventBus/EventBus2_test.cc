...
bool executeNew( const std::string& arg )
{
  ec.count( "NewEmail" );
  return true;
}
bool executeTrash( const std::string& arg )
{
  ec.count( "TrashEmail" );
  return true;
}
bool executeReply( const std::string& arg )
{
  ec.count( "ReplyEmail" );
  return true;
}
bool executeAttachment( const std::string& arg )
{
  ec.count( "AddAttachment" );
  return true;
}

TEST(EventBusTest20, BasicSingleThreaded) 
{
  ec.reset();
...
bool executeAttachmentWithCancel( const std::string& arg )
{
  ec.count( "AddAttachment" );
  return false;
}
bool executeAttachmentLost( const std::string& arg )
{
  ec.count( "executeAttachmentLost" );
  return true;
}

TEST(EventBusTest21, BasicSingleThreadedWithCancel) 
{
  ec.reset();
  EXPECT_EQ( ec.get_count( "NewEmail" ), 0);
  EXPECT_EQ( ec.get_count( "TrashEmail" ), 0);
  ...
  eb.add("ReplyEmail", executeReply);
  eb.add("AddAttachment", executeAttachment);
  eb.add("AddAttachment", executeAttachmentWithCancel);
  eb.add("AddAttachment", executeAttachmentLost);
  ...
  EXPECT_EQ( ec.get_count( "TrashEmail" ), 1 );
  EXPECT_EQ( ec.get_count( "ReplyEmail" ), 0 );
  EXPECT_EQ( ec.get_count( "AddAttachment" ), 2 );
  EXPECT_EQ( ec.get_count( "executeAttachmentLost" ), 0 );
}
...
TEST(EventBusTest25, BasicMultiThreaded) 
{
  eb.reset();
  ac.reset();
  ec.reset();

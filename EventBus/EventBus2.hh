...
/** 
 * EventHandler is a function accepting a string and argument returning bool.
 * If false is returned, other event handlers are NOT called.
 */
typedef std::function<bool( const std::string&  )> EventHandler;
...
public:
     ...
     * Event handler can cancel subsequent events.
     */
    void invoke( const std::string& ename, const std::string& arg ) // const
    {
        std::lock_guard<std::mutex> g(_m);
        ...
        if(result != _ename2handlers.end())
            for( auto handler: result->second )
                if( not handler( arg ) )
                    break; // event handler cancelled subseq events
    }
